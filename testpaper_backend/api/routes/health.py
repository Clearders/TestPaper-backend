...
@router.get("/postgres")
def postgres_health(request: Request):
    try:
        if engine is None:
        ...
        result = {"status": "connected", "latencyMs": latency_ms}
        if not is_production():
            result["postgresVersion"] = version
        return envelope(result, request)
    except Exception as exc:
        error_msg = "Database health check failed" if is_production() else str(exc)
        return JSONResponse(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            content=envelope({"status": "disconnected", "error": error_msg}, request),
        )
